...
import { submitTicketSatisfactionValidator } from '#validators/ticket'

export default class TicketsController {
  async index({ inertia, auth }: HttpContext) {
    const user = auth.getUserOrFail()

    const [awaiting, mine, options] = await Promise.all([
  ...
  async resolve({ params, auth, response, session }: HttpContext) {
    ...
    await markTicketResolved(ticket, client)
  ...
  async confirmClose({ params, request, inertia }: HttpContext) {
    ...
    const client = ticket.issuerType === 'user' ? await User.find(ticket.issuerId) : null
    await closeTicketAfterClientConfirmation(ticket, client)
  ...
  private async getFormOptions() {
    const [priorities, categories, groups, technicians] = await Promise.all([
